...
class CrowdAPI {
    ...
    {
        $crowdURL = 'http://issues.unep.org/crowd';
        $crowdAppName = 'unepmap repository';
        $crowdAppPassword = '123456';
        $crowdHTTPHeaders = [
    ...
    public function canUserLogin($username)
    {
        ...
        if (count($userGroups) > 0) {
            if (count(array_intersect($userGroups, 'group')) > 0) {
                return true;
            }
        }
        return false;
